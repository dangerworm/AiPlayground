class ModelConfig:
  """Constants for the DeepSeek-R1:1.5b model."""
...

{
  ...
  "environment": string e.g. "Another creature, exactly the same form as you stands at (3,4)",
...
Move(dx as int, dy as int): Move in the direction specified by dx and dy.
Speak(speech as string): Speak the string aloud.
Think(): Continue your internal monologue without taking action.
Ask(question as string): Ask a question to the system for further information.
Recall(query as string): Recall a memory from the vector database.
Remember(memory as string): Remember a fact.
"""